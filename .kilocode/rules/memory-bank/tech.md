...
- **Build & Linting**: pnpm workspaces for monorepo management, TypeScript (<5.2.0) for compilation, ESLint (v9) and Prettier (v2.7.1) for code quality.
...
## Development Setup
1. **Prerequisites**: Node.js (with corepack enabled), pnpm, Docker (for PostgreSQL and RabbitMQ), Caddy (via Homebrew on macOS), tmuxinator.
2. **Installation**: Run `pnpm` in root to install dependencies across workspaces.
3. **Build**: `pnpm build` compiles all packages (TypeScript to dist/).
4. **Database Setup**: `cd packages/database && docker-compose up -d && sleep 30 && pnpm knex migrate:latest` (creates schema, tables, demo data).
5. **Event Bus**: `cd packages/event-bus && docker-compose up -d`.
6. **Services**: Run each with `pnpm dev` (e.g., `cd packages/movies && pnpm dev` on port 4000; similar for Rental:4001, Accounting:4002; Pricing has no HTTP port).
7. **Proxy**: `cd packages/reverse-proxy && caddy run`.
8. **Frontend**: `cd packages/frontend && pnpm dev` (runs on http://localhost:3000).
9. **Automated Start**: `tmuxinator start` launches all via .tmuxinator.yml.
10. **Testing**: e.g., `cd packages/pricing && pnpm test` for integration tests.
...
- **Dependencies**: Workspace-linked packages (e.g., `@ddd-video-club-v2/database`); updates require `pnpm` rebuild.
